...
import { buildTimetableViewModel, resolveCurrentWeek } from './utils/timetable-normalizer.js';
import { createTimetableGrid, initTimetableGridScale } from './components/TimetableGrid.js';
import { createTimetableDayList } from './components/TimetableDayList.js';
import { createLiveTimetableStatus, initLiveTimetableStatus } from './components/LiveTimetableStatus.js';
import { createTimetableVisualEditor, initTimetableVisualEditor } from './components/TimetableVisualEditor.js';
...
async function loadTimetableData() {
  ...
  try {
    ...
    // 更新页面标题
    document.getElementById('table-name').textContent = currentViewModel.tableName;
    ...
    errorTextEl.textContent = error instanceof Error ? error.message : '课表数据加载失败';
...
function renderTimetable() {
  ...
  // 更新周次显示
  document.getElementById('current-week').textContent = currentViewModel.currentWeek;
  ...
  currentWeekBadge.style.display = currentViewModel.currentWeek === actualCurrentWeek ? 'inline-block' : 'none';
  ...
  visualEditorContainer.innerHTML = createTimetableVisualEditor(currentViewModel, baselineParsed);
  initTimetableVisualEditor(visualEditorContainer.querySelector('.visual-editor'), currentViewModel, baselineParsed);
  ...
  // 渲染日列表（移动端）
  const dayListContainer = document.getElementById('timetable-daylist-container');
